...
namespace li3_assetic\extensions;
...
class Process {
	 ...
	 * @api
	 */
	public function __construct($commandline, $cwd = null, array $env = null, $stdin = null,
		...
		$this->options = array_replace(array('suppress_errors' => true, 'binary_pipes' => true),
			$options);
	}
	...
	public function start($callback = null) {
		...
		$this->process = proc_open($commandline, $descriptors, $this->pipes, $this->cwd, $this->env,
			$this->options);
	...
	public function wait($callback = null) {
		...
		if ($this->processInformation['signaled']) {
			throw new \RuntimeException(sprintf('The process stopped because of a "%s" signal.',
				$this->processInformation['stopsig']));
		}
		...
		return $this->exitcode = $this->processInformation['running'] ? $exitcode :
	...
	public function getExitCodeText() {
		...
		return isset(self::$exitCodes[$this->exitcode]) ? self::$exitCodes[$this->exitcode] :
				'Unknown error';
	}
	...
	public function stop($timeout = 10) {
		...
		if ($this->isRunning()) {
			...
			$exitcode = proc_close($this->process);
			$this->exitcode = -1 === $this->processInformation['exitcode'] ? $exitcode :
					$this->processInformation['exitcode'];
		}
